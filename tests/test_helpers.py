"""Unit tests for the pydantic-backed helper functions in hivemind_ma_player.
...
from hivemind_ma_player import (  # noqa: E402  (imports after stub setup)
...
# ---------------------------------------------------------------------------
# HiveMind-specific: poll response payload unwrap
# ---------------------------------------------------------------------------
...
class TestPollPayloadUnwrap(unittest.TestCase):
    ...
    def _unwrap(self, resp):
        """Mirror the unwrap logic from HiveMindPlayer.poll()."""
        inner = resp.payload if hasattr(resp, "payload") else resp
